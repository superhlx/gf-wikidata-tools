...
bengali_place = grammar_place.languages['DescriptionsBen']
...
#     """
#     Determine the high-level type of a Wikidata entity by checking its P31 claims
#     against local QID collections for common types.
#     """
#     claims = entity.get('claims', {})
#     p31s = claims.get('P31', [])
#     p31_qids = [
#         snak.get('mainsnak', {}).get('datavalue', {}).get('value', {}).get('id')
#         for snak in p31s if snak.get('mainsnak', {}).get('datavalue', {}).get('value', {}).get('id')
#     ]
#     if 'Q5' in p31_qids:
#         return 'human'
#     if 'Q515' in p31_qids:
#         return 'city'
#     if any(qid in university_types for qid in p31_qids):
#         return 'university'
#     if 'Q23442' in p31_qids:
#         return 'island'
#     if 'Q23397' in p31_qids:
#         return 'lake'
...
def get_wikidata_entity(qid, max_retry=5):
    url = f'https://www.wikidata.org/wiki/Special:EntityData/{qid}.json'
    for attempt in range(max_retry):
        try:
            time.sleep(random.uniform(1.5, 2.5))  # 控制整体速率
            resp = requests.get(url, timeout=10)
            if resp.status_code != 200:
                print(f"[WARN] {qid} returned status {resp.status_code}, retrying...")
                time.sleep(5)
                continue
            if not resp.text.strip():  # 空响应
                print(f"[WARN] {qid} returned empty response, retrying...")
                time.sleep(5)
                continue
            data = resp.json()
            return data['entities'][qid]
        except json.JSONDecodeError as e:
            print(f"[ERROR] JSON decode failed on {qid}: {e}")
            print(f"[DEBUG] Response was: {resp.text[:200]}")
            time.sleep(10)
        except Exception as e:
            print(f"[ERROR] failed on {qid}: {e}")
            time.sleep(10)
    raise RuntimeError(f"Failed to fetch {qid} after {max_retry} retries")
...
def build_university_expr(entity, language='eng'):
    ...
    if year:
        if language == 'ben':
            year = english_to_bangla_number(year)
...
def build_description(qid):
    ...
    name_bn = get_label(entity, 'bn') or name
    ...
    if entity_type == 'human':
        ...
        expr = build_university_expr(entity, language="chi")
        zh = chinese_place.linearize(expr)
        expr = build_university_expr(entity, language="eng")
        en = english_place.linearize(expr)
        expr = build_university_expr(entity, language="ben")
        ben = bengali_place.linearize(expr)
        return name_zh, name_en, name_bn, zh, en, ben
...
if __name__ == "__main__":
    ...
    qid = sys.argv[1]
    try:
        name_zh, name_en, name_bn, zh, en, ben = build_description(qid)
        print(f"{name_zh}：{zh}")
        print(f"{name_en}: {en}")
        print(f"{name_bn}: {ben}")  
    except Exception as e:
