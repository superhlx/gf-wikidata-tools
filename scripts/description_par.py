...
def get_wikidata_entity(qid, max_retries=10, base_delay=1):
    """
    Get Wikidata entity with retry mechanism.
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'entities' in data and qid in data['entities']:
                ...
                print(f"Warning: No entity data found for {qid}, attempt {attempt + 1}")
                
        except (requests.exceptions.RequestException, json.JSONDecodeError, KeyError) as e:
            print(f"Error fetching {qid} (attempt {attempt + 1}): {e}")
    ...
    raise Exception(f"Failed to fetch entity {qid} after {max_retries} attempts")
...
def read_jsonl_file(file_path):
    """
    Read JSONL file and extract QIDs from URLs
    """
    qids = []
    with open(file_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            try:
                ...
                url = data.get('url', '')
                qid = extract_qid_from_url(url)
                if qid:
                    qids.append(qid)
                else:
                    print(f"Warning: Could not extract QID from URL '{url}' at line {line_num}")
...
def save_results(results, output_file):
    """
    Save results to a file in JSONL format
    """
...
def process_batch(input_file, output_file):
    """
    Process all QIDs from the input file and save results in specified format.
    """
    print(f"Reading QIDs from {input_file}...")
    # 这里改成读取原始URL和QID的对
    url_qid_pairs = []
    with open(input_file, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            try:
                ...
                qid = extract_qid_from_url(url)
                if qid:
                    url_qid_pairs.append((url, qid))
                else:
                    print(f"Warning: Could not extract QID from URL '{url}' at line {line_num}")
            except json.JSONDecodeError as e:
                print(f"Error parsing JSON at line {line_num}: {e}")
    print(f"Found {len(url_qid_pairs)} QIDs to process")
    
    results = []
    failed_qids = []
    
    for i, (url, qid) in enumerate(url_qid_pairs, 1):
        print(f"\nProcessing {i}/{len(url_qid_pairs)}: {qid}")
        try:
            entity = get_wikidata_entity(qid)
            orig_zh = entity.get('descriptions', {}).get('zh', {}).get('value', '')
            orig_en = entity.get('descriptions', {}).get('en', {}).get('value', '')
            name_zh, name_en, gen_zh, gen_en = build_description(qid)
            result = {
                "url": url,
                "orig_zh": orig_zh,
                "orig_en": orig_en,
        ...
        # Save results periodically (every 10 items) to avoid data loss
        if i % 10 == 0:
    ...
    print(f"\nProcessing complete!")
    print(f"Total processed: {len(url_qid_pairs)}")
    print(f"Successful: {len(url_qid_pairs) - len(failed_qids)}")
    print(f"Failed: {len(failed_qids)}")
    if failed_qids:
        print(f"Failed QIDs: {', '.join(failed_qids)}")
...
if __name__ == "__main__":
    ...
    input_file = '/home/heluxue/gf-wikidata-tools/scripts/split_0/City_split.jsonl'
    output_file = 'City_descriptions.jsonl'
